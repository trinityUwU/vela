...
│   ├── App.tsx                     Layout + modals (menu, rename, delete, props, newfolder)
...
│   │
│   ├── hooks/
│   │   ├── useFileManager.ts       État central : nav, mode, sélection, CRUD, moveEntry
...
│       ├── FileGrid.tsx            Grille mode Fichiers, drag & drop
│       ├── FileList.tsx            Liste pane gauche mode Édition, drag & drop
...
│       ├── PropertiesModal.tsx     Métadonnées + contenu dossier + app par défaut modifiable
│       ├── ContextMenu.tsx         Ouvrir / Renommer / Supprimer / Propriétés
│       ├── SearchBar.tsx           SearchInput (topbar) + SearchResults (overlay)
│       ├── InputModal.tsx          Modal saisie texte (renommer, nouveau dossier)
│       ├── ConfirmModal.tsx        Confirmation suppression
│       └── icons.tsx               SVG inline (ArrowUp, Refresh, Eye, Search, Save…)
│
└── src-tauri/
    ├── Cargo.toml                  zip, tar, flate2, bzip2, xz2, base64
    ...
    └── src/
        ├── main.rs
        ├── lib.rs                  Builder + enregistrement 20 commandes
        ├── fs_ops.rs               CRUD + chunks + search + move + props + open_native
        ...
        └── apps.rs                 get_apps_for_file, set_default_app (xdg-mime + .desktop scan)
```
