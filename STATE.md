...
- **Affichage fichier sans switch de profil** : `open_file`/`preview_content` affichent dans la **zone éditeur du profil COURANT** (`fm.setOpened`, les onglets dérivent de `opened`) — plus de `switchToEdition`. Si le profil actif n'a **aucune zone éditeur**, le Rust renvoie une **erreur explicite à Claude** (via `profiles::active_editor_status()` qui lit `profiles.json`, source de vérité persistée). Le clic terminal suit la même règle avec fallback switch Édition.
- **Descriptions tools impératives** (mcp-bridge.ts) : `open_file`/`open_url` = « UNIQUE bonne façon », interdiction explicite de Playwright, interdiction de recopier le contenu du fichier dans le chat → Claude choisit les bons tools sans qu'on insiste.
- **Contexte Vela dans le system prompt** : le wrapper ajoute `--append-system-prompt "$(cat assets/vela-context.md)"` (fichier embarqué via `include_str!`). Claude sait qu'il tourne dans Vela, quand invoquer le MCP `vela`, et que tous ses autres MCP/skills restent dispos. **Aucune config globale touchée** (CLAUDE.md/MCP/skills intacts) — isolation préservée, scopé au wrapper Vela uniquement.
- **Terminal en zone bottom** : `ZoneLayout` rend la zone `bottom` redimensionnable (`ResizeHandle` + hauteur dynamique clamp 120..vh-160) et enveloppe le panneau dans `flex-1 min-h-0` comme le dock legacy → resize + scroll xterm OK (avant : `h-64` fixe sans poignée).
